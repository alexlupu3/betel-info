...
- **`apps/web/`** — React/Vite PWA for *Biserica Betel Cluj*. Displays church events, announcements, and community info fetched from two static JSON files. Fully themeable, works offline, mobile-first.
- **`apps/admin/`** — Laravel 12 + Breeze PHP admin app. Will replace static JSON files with a DB-backed CMS with secure authentication. **Currently in early scaffolding — JSON files remain the source of truth.**
...
| `apps/web/public/page.json` | Branding: logo, title, description, theme colors |
...
| Path | Purpose |
|---|---|
| `apps/admin/app/Models/Location.php` | Location model |
| `apps/admin/app/Models/PageConfig.php` | Page config model |
...
| Table | Purpose |
|---|---|
| `locations` | Church locations (slug, name, logo_path) |
| `page_configs` | Branding per location (nullable location_id = global default) |
...
Theme is defined in `apps/web/public/page.json` and injected at runtime as CSS custom properties on `:root`:
...
## Page Config (`apps/web/public/page.json`)
